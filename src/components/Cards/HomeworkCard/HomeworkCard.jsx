  ...
  return (
    <div style={bg} className="rounded-[15px] overflow-hidden p-[2px]">
      <div className="bg-[#11072D] p-5 rounded-[15px] flex flex-col gap-3 text-[#9255F5]">
        <span className="font-[700] text-[18px] xss:text-[20px] lg:text-[28px] block text-left">
          {homework?.subject}
        </span>
        <div className="text-[#9255F5] text-opacity-80 text-[15px] xss:text-[17px] lg:text-[22px] font-[400] flex flex-col items-start gap-0">
        ...
        <div className="flex flex-row flex-wrap gap-3 justify-between items-center">
          <span className="text-[#9255F5] text-opacity-90  text-[15px] xss:text-[17px] lg:text-[22px] font-[400]">
            {moment(homework?.dueDate).format("DD MMM Y")}
          </span>
          <Link
            state={location}
            to={`/homework/${homework?._id}`}
            className="toButton homeworksToButton ml-auto"
          >
            <FaChevronRight />
          </Link>
        </div>
